...
| `backtest.py` | `/api/backtest/*` | KIS AI Extensions MCP 연동 백테스트: 프리셋/커스텀/배치 실행, 결과 조회, 이력 삭제. `KIS_MCP_ENABLED=true` 필요. 8개 엔드포인트. 이력에 종목명(`symbol_name`) 부착. 프리셋 params 오버라이드 |
