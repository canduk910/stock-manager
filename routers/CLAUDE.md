...
| `quote.py` | `WS /ws/quote/{symbol}`, `WS /ws/execution-notice`, `WS /ws/market-status`, `GET /api/quote/us/{symbol}/orderbook`, `GET /api/quote/us/{symbol}/detail` | 실시간 호가 WebSocket (KR/FNO/US) + 체결통보 WS + **(2026-05-08) 장운영정보 통합 멀티플렉스 WS**(H0UNMKO0/H0STMKO0/H0NXMKO0). `/ws/quote/{symbol}` 에 `exchange=auto\|UN\|KRX\|NXT` 쿼리 추가(기본 `auto` — 백엔드 KISQuoteManager 가 KST 시계 기반 4구간 자동 분기). **(2026-05-08 해외 호가/상세 신규)** US REST 폴백 엔드포인트 2종 — `/api/quote/us/{symbol}/orderbook` (HHDFS76200100, 10단계 호가) + `/api/quote/us/{symbol}/detail` (HHDFS76200200, 시/고/저/거래량/52주). `Depends(get_current_user)` + `kis_overseas_client.get_kis_orderbook`/`get_kis_price_detail` 위임. 응답 200/503(KIS 키 부재)/504(KIS 응답 None). 해외 WS 분기는 dict 메시지를 그대로 forward → `{type:"orderbook", ...}` 자동 통과. |
