/**
 * 증권사별 목표가 + 리포트 표 (공용 컴포넌트).
 *
 * AnalystReportsModal(팝업)과 DetailPage 재무분석 탭(인라인) 양쪽에서 재사용.
 * 데이터 소스: /api/advisory/{code}/analyst-reports — naver_research 스크래핑 (KR) /
 *   yfinance upgrades_downgrades (US). **GPT 호출 0건**.
 *
 * Props:
 *   code, market, currentPrice (옵션 — 평균 목표가 대비 갭% 계산),
 *   compact (옵션 — 인라인용 작은 표시)
 */
...
export default function AnalystReportsTable({ code, market = 'KR', currentPrice = null, compact = false }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="ml-2 text-sm text-gray-500">증권사 컨센서스 조회 중...</span>
      </div>
    )
  }
  ...
  if (reports.length === 0) {
    return <div className="text-sm text-gray-400 py-6 text-center">증권사 리포트가 없습니다.</div>
  }

  return (
    <div className="space-y-2">
      {/* 평균 목표가 요약 (KR 한정, 인라인용 상단 표시) */}
      {isKR && avgTarget && (
        <div className={`flex items-center justify-between text-sm px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200`}>
          <span className="text-gray-700">
            🇰🇷 한국 증권사 평균 <strong className="text-gray-900">{avgTarget.toLocaleString()}원</strong>
            <span className="text-gray-500 ml-1">({pricesWithTarget.length}개 발행 PDF, 네이버 리서치)</span>
          </span>
          {gap && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50 text-gray-600">
                ...
                {!compact && <th className="px-2 py-2 text-left font-medium">리포트</th>}
                <th className="px-2 py-2 text-center font-medium">날짜</th>
                <th className="px-2 py-2 text-center font-medium w-10">PDF</th>
              </tr>
            </thead>
            <tbody>
              {reports.map((r, i) => (
                <tr key={i} className="border-b last:border-b-0 hover:bg-gray-50/50">
                  ...
                  {!compact && (
                    <td className="px-2 py-2 text-gray-600 truncate max-w-[260px]" title={r.title}>{r.title}</td>
                  )}
                  <td className="px-2 py-2 text-center text-gray-500 whitespace-nowrap text-xs">{r.date}</td>
                  <td className="px-2 py-2 text-center">
                    {r.pdf_url ? (
                      <a href={r.pdf_url} target="_blank" rel="noopener noreferrer"
                         className="text-indigo-500 hover:text-indigo-700" title={r.title || 'PDF 리포트'}>
                        📄
                      </a>
                    ) : (
