...
const PRESETS = [
  ...
  { value: 'greenblatt', label: '마법공식 (조엘 그린블라트)',
    desc: 'ROIC + Earnings Yield 합산 순위', dart: true, sort: 'mktcap',
    detail: '조엘 그린블라트의 마법공식. 높은 투하자본수익률(ROIC)과 높은 이익수익률(EY = EBIT/EV)을 동시에 가진 종목을 찾습니다. 두 순위를 합산하여 Combined Rank가 가장 낮은 종목이 최상위입니다. 금융/유틸리티/바이오 업종은 원저에서 제외를 권고합니다.' },
  { value: 'neff', label: '저PER 총수익 (존 네프)',
    desc: '(EPS CAGR + 배당) / PER', dart: true, sort: 'mktcap',
    detail: '존 네프의 저평가 성장 공식. (EPS 성장률 + 배당수익률) / PER 비율이 높은 종목을 찾습니다. 성장과 배당을 합산한 총수익 대비 PER이 낮을수록 매력적입니다. Neff Ratio > 2.0이면 매우 매력적, > 1.0이면 매력적입니다.' },
  { value: 'seo', label: '기대수익률 (서준식)',
  ...
  { value: 'ncav', label: '순유동자산 (벤저민 그레이엄)',
    desc: '청산가치 > 시총인 종목', dart: true, sort: 'mktcap',
    detail: '벤저민 그레이엄의 NCAV 전략. 유동자산에서 총부채를 뺀 순유동자산가치(NCAV)가 시가총액보다 큰 종목을 찾습니다. 시총 <= NCAV x 2/3이면 최고 등급. 적자기업도 포함됩니다. 가장 보수적인 절대 안전마진 공식.' },
  { value: 'fisher', label: '저PSR 턴어라운드 (켄 피셔)',
    desc: '매출 대비 저평가 턴어라운드', dart: true, sort: 'mktcap',
    detail: '켄 피셔의 PSR(주가매출비율) 전략. 시가총액 / 매출액이 낮은 종목을 찾습니다. PSR < 0.75이면 강력매수. 일시적 이익 훼손으로 주가가 급락했지만 매출이 건재한 턴어라운드 후보 발굴에 특화.' },
  { value: 'piotroski', label: '재무건전성 (조셉 피오트로스키)',
    desc: '재무건전성 8점 채점', dart: true, sort: 'mktcap',
    detail: '조셉 피오트로스키의 F-Score. 수익성(4항목) + 재무구조(2항목) + 영업효율(2항목) = 8점 만점. 7~8점이면 재무 최상위, 3점 이하면 절대 매수 금지. 저PBR/저PER 종목의 가치 함정을 걸러내는 품질 필터.' },
]
