...
| `useQuote.js` | 실시간 호가 WS. `useQuote(symbol, market='KR', exchange='auto')`. exchange 쿼리 자동 부착. rAF throttle 자체 관리. `type:"error"` 메시지 수신 시 `errorMessage` state 노출 (백엔드 KIS 키 미설정 등 silent failure 차단). **2026-05-18 결함 수정**: query 파라미터를 path 안에서 먼저 합친 후 `buildWsUrl` 한 번만 호출 — 이전엔 buildWsUrl이 `?token=...` 부착한 base에 또 `?exchange=auto`를 이어붙여 URL의 `?`가 두 번 등장 → 백엔드가 token 값에 `?exchange=auto` 흡수 → JWT 서명 검증 실패로 1008 무한 close (HTTP API는 Authorization 헤더라 무영향) |
...
| `useMarketClock.js` | KST 4구간 KR 거래소 자동 판정 (`UN`/`KRX`/`NXT`). `resolvePhaseByClock(now)` 순수 함수 export. 1분 setInterval **단독 (WS override 제거 — `/ws/market-status` 폐지로 시계 폴백만 사용)**. **2026-05-18 결함 수정**: `now.getTimezoneOffset()` 부호 합산 트릭이 KST 브라우저(`offset=-540`)에서 9시간을 뒤로 돌려 평일 14:50을 350분(CLOSED)로 잘못 분류 → 호가창 "통합(휴장)" 오라벨 + 시세판 폴링 주기 60s 잘못 분기. `Intl.DateTimeFormat({timeZone:'Asia/Seoul'})` 패턴(useUsMarketClock과 일관)으로 교체 |
