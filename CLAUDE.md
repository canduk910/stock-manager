...
`requirements.txt`: `websockets`, `sqlalchemy`, `alembic`, `pytest`, `psycopg2-binary`, `pyyaml`, `cryptography>=42` (KIS 자격증명 AES-GCM), `apscheduler` (매크로 cleanup cron).

미포함: `pycryptodome` — `KoreaInvestmentWS` 체결통보 AES 복호화용 (별도 설치).
