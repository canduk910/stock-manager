...
**사용자별 KIS 자격증명 (멀티 계좌):** `KIS_ENCRYPTION_KEY` (AES-GCM 32-byte b64. SSM SecureString. **분실 시 모든 사용자 KIS 자격증명 영구 복구 불가**), `KIS_VALIDATION_TTL_HOURS`(기본 24). 사용자 1명이 N개 KIS 계좌(주식/연금/CMA 등) 등록 가능 — `user_kis_credentials` PK `id` AI + `UNIQUE(user_id, label)` + `is_default` 플래그. 잔고 합산/계좌별 주문/체결통보 라벨링은 `account_label`로 분기.

**외부 데이터:** `OPENDART_API_KEY`(국내 공시/재무), `FINNHUB_API_KEY`(미국 실시간 시세, 옵션), `FRED_API_KEY`(매크로 OAS 폴백 권장), `SEC_EDGAR_USER_AGENT_CONTACT`(SEC EDGAR User-Agent 연락처 이메일 필수, 미설정 시 SEC 정책상 차단), `AI_IPO_TICKERS`(반도체 AI IPO 추적 화이트리스트, 콤마 구분 예: `"RDDT,ARM"`, 기본 빈 문자열).
...
- 반도체 사이클 모니터링 (indicator_values, signals, semiconductor_thresholds — 2026-06-13 신설, Phase 1)
...
- CI/CD: `main` push → pytest + frontend build → Docker → ECR → backtester MCP health → EC2 자동 배포 (5m timeout)
  - **hang 가드 (2026-06-20)**: pytest `timeout=120`(pytest-timeout, 개별 테스트 무한 대기 차단) + job `timeout-minutes`(test 15/frontend 10/docker 20/deploy 25) + `concurrency`(CI는 `cancel-in-progress=true`로 hang run 누적 차단, Deploy는 `false`로 배포 중단 회피). 외부 네트워크 mock 누락 테스트가 socket에서 멈춰도 CI 무한 hang 안 됨.
...
`requirements.txt`: `websockets`, `sqlalchemy`, `alembic`, `pytest`, `psycopg2-binary`, `pyyaml`, `cryptography>=42` (KIS 자격증명 AES-GCM), `pycryptodome>=3.20` (`wrapper.py` 모듈 레벨 `from Crypto.Cipher import AES` — KIS 체결통보 H0STCNI0 복호화. 누락 시 `wrapper.py` import 자체가 ModuleNotFoundError로 실패해 CI/배포 차단), `apscheduler` (매크로 cleanup cron), `pytest-timeout` (CI 개별 테스트 hang 가드 — pytest.ini `timeout=120`. 누락 시 네트워크 mock 누락 테스트가 CI를 무한 hang).
