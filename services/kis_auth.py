...
balance/order 라우터·서비스, stock 게이트웨이 등에서 공용으로 사용한다.

2026-07-04 (refactor F-1): routers/_kis_auth.py → services/kis_auth.py 이동.
services·stock 레이어가 routers를 역방향 import하던 구조 교정.
routers/_kis_auth.py는 sys.modules 별칭 shim — 두 경로는 동일 모듈 객체이므로
기존 import/monkeypatch 경로 모두 유효하다.
