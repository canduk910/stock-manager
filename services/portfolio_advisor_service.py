...
def _compute_cache_key(balance_data: dict, user_comment: str | None = None) -> str:
    """보유종목 구성(code+quantity) + 사용자 코멘트로 SHA256 캐시 키 생성.

    종목 코드와 수량 조합만으로는 동일 보유 구성이면 가격 변동과 무관하게
    캐시를 재사용한다 (30분 TTL 내). sort()로 순서 독립적 일관성을 보장한다.

    user_comment(2026-05-07): 동일 포트폴리오라도 사용자 코멘트가 다르면 새 보고서 생성.
    None과 ""(빈/공백) 은 strip 후 같은 키로 처리(백워드 호환).
    """
    ...
    for s in balance_data.get("overseas_list") or []:
        pairs.append((s.get("code", ""), str(s.get("quantity", ""))))
    pairs.sort()
    payload = {"pairs": pairs, "comment": (user_comment or "").strip()}
...
_REGIME_CASH_RULES = {
    ...
    "selective":    ("35%", "65%", "선별 투자. 체제+사이클 조합 권고에 따라 현금/주식 비중 조정."),
    "cautious":     ("50%", "50%", "방어 우위. 체제+사이클 조합 권고에 따라 현금/주식 비중 조정."),
    # defensive: 사이클이 회복/확장이면 사이클 주도 섹터에 한정 부분 매수 가능 (cycle×regime 매트릭스 참조)
    "defensive":    ("75%", "25%", "방어 기조이나 사이클 회복/확장 시 주도 섹터 한정 부분 매수 검토."),
}
...
def _get_macro_news_context() -> list[str]:
    """매크로 뉴스 헤드라인 수집 — 한국 5개 + 해외 3개. 실패 시 빈 리스트."""
    try:
        from services import macro_service
        news = macro_service.get_news()
        headlines: list[str] = []
        for article in (news.get("korean") or [])[:5]:
            title = article.get("title", "").strip()
            if title:
                headlines.append(title)
        for article in (news.get("international") or [])[:3]:
            title = article.get("title_ko") or article.get("title_original", "")
            if title.strip():
                headlines.append(title.strip())
        return headlines
    except Exception as e:
        logger.debug("매크로 뉴스 수집 실패: %s", e)
        return []
...
def _format_cycle_context(cycle_ctx: dict | None) -> str:
    """경기 사이클 국면 정보를 시스템 프롬프트용 문자열로 포맷.

    체제+사이클 조합 액션 매트릭스 가이드 1줄 추가 (2026-05-01).
    """
    if not cycle_ctx or not cycle_ctx.get("phase"):
        return ""
    label = cycle_ctx.get("phase_label", cycle_ctx["phase"])
    conf = cycle_ctx.get("confidence", "?")
    sectors = ", ".join(cycle_ctx.get("leader_sectors", []))
    desc = cycle_ctx.get("phase_desc", "")
    matrix_guide = (
        "\n[조합 액션 매트릭스] 확장기+defensive 체제: 가치주 일부 정리 + 주도 섹터 신규 진입 5~10% 허용 / "
        "회복기+cautious: 주도 섹터 분할 매수 가능 / 수축기+defensive: 신규 매수 금지·매도 우선 / "
        "과열기+accumulation: 안전마진 25%+ 종목만 매수."
    )
    return (
        f"경기 사이클 국면: {label} (신뢰도 {conf}%). {desc}. 주도 섹터: {sectors}. "
        f"섹터 추천 시 경기 사이클 주도 섹터를 우선 고려하세요.{matrix_guide}"
    )
...
def _build_system_prompt(
    ...
    user_comment(2026-05-07): 사용자 가설 양면 평가 가이드 블록을 후미에 append.
    advisory_service와 동일 규칙(악마의 변호인 / strength 1~10 / overall_stance 5단계).
    """
    cash_min, stock_max, cash_guidance = _REGIME_CASH_RULES.get(regime, _REGIME_CASH_RULES["selective"])
    per_max, pbr_max = _REGIME_VALUATION_LIMITS.get(regime, (15, 1.5))
    contrarian_section = _CONTRARIAN_RULES if regime in ("cautious", "selective") else ""
    cash_warning = ""
    cycle_phase = (cycle_ctx or {}).get("phase") if cycle_ctx else None
    if regime == "defensive" and cycle_phase == "contraction":
        cash_warning = f"\n【중요】 defensive+수축기: 현재 현금 {cash_pct}% / 권고 {cash_min}. 신규 매수 금지, 매도/관망 우선."
    elif regime == "defensive":
        cash_warning = (
            f"\n【참고】 defensive 체제이나 사이클이 {cycle_phase or '미확인'}임. "
            f"현재 현금 {cash_pct}% / 권고 {cash_min}. 사이클 주도 섹터 한정 부분 매수(권고치 30~50%) 검토 가능."
        )
    elif cash_pct < float(cash_min.replace('%', '')):
        cash_warning = (
            f"\n현재 현금 {cash_pct}%는 {regime} 체제 권고({cash_min}) 대비 부족. "
            f"체제+사이클 조합 권고에 따라 현금/주식 비중 조정 검토."
        )
    ...
    if cmt:
        user_comment_block = (
            ...
            "4. summary는 1~3문장으로 사용자에게 직접 답하라.\n"
            "5. 사용자 의견과 본 보고서 진단/리밸런싱/매매안은 모순 없이 정합하라 — 본문 결론과 stance가 어긋나면 본문을 우선하고 stance를 보정하라.\n"
            "   diagnosis/rebalancing/trades 결정은 코멘트와 무관하게 데이터 기반으로 산출하며, stance만 가설 평가용으로 사용하라.\n"
...
{{
  ...
  "sector_recommendations": [
    {{
      "sector": "섹터명 (예: 반도체, 헬스케어, 에너지)",
      "rationale": "매크로 환경 + 포트폴리오 분산 기반 추천 근거 (2~3문장)",
      "target_weight_pct": 포트폴리오 내 목표 비중(%),
      "representative_stocks": [
        {{"stock_name": "종목명", "stock_code": "종목코드", "market": "KR 또는 US", "reason": "해당 종목 선정 근거"}}
      ],
      "entry_timing": "immediate 또는 this_week 또는 this_month",
      "entry_strategy": "분할 매수 등 진입 전략"
    }}
  ],
  "market_context": "현재 시장 상황에 대한 간략한 코멘트",
  "user_commentary_evaluation": null 또는 {{"user_comment":"<원문>", "overall_stance":"strong_agree|agree|balanced|disagree|strong_disagree", "agree_points":[{{"point":"...","evidence":"...","strength":1~10}}], "disagree_points":[{{"point":"...","evidence":"...","strength":1~10}}], "summary":"1~3문장"}},
...
G. 포트폴리오 가중 평균 등급이 B 미만(weighted_avg score<16) 또는 C/D 등급 종목이 과반이면 신규 편입은 **사이클 주도 섹터 + 가치 B 이상 또는 성장 G-A 한정**으로 제한 (전면 보류 아님). 기존 C/D 등급 종목은 우선 정리 권고 (체제와 무관). D 등급이지만 성장 보조 G-A 종목은 분할 매수 검토 가능 (factor=0.30, 손절 -20%).
...
손실 종목 처리 (3안 균형 제시):
13. -7%~-15% 주의: 재무 건전성 재점검 후 ① 물타기 ② 홀딩 ③ 부분 손절 중 1안 명확히 선택 (PBR<1.0+부채비율<100%면 물타기 우선 검토)
14. -15% 초과 위험: ① 손절 ② 평균단가 분할매수(물타기) ③ 홀딩 3안을 모두 평가하여 종목 특성에 맞춰 선택. 물타기 조건은 PBR<0.7+부채비율<100%+FCF양수일 때만 허용.
15. 고평가(PER>업종평균1.5배 또는 PBR>2.0) 손실 종목은 비중 축소/손절 우선 검토. 단 성장 보조 G-A 종목은 catalyst 유효성 재검토 후 홀딩 가능.
...
신규 섹터 진입 추천 (sector_recommendations):
29. 체제+사이클별 신규 섹터 추천 한도 (사이클 주도 섹터 우선):
    - accumulation: 최대 3~5개 섹터, 섹터당 대표 종목 2~3개
    - selective: 최대 2~3개 섹터, 섹터당 대표 종목 1~2개
    - cautious: 최대 1~2개 섹터(사이클 주도 섹터 우선), 대표 종목 1~2개
    - defensive+수축기: 신규 섹터 진입 금지 (sector_recommendations = [])
    - defensive+회복/확장: 사이클 주도 섹터 1개로 한정, 대표 종목 1개 (권고치 30~50%)
30. 추천 섹터 선정 기준:
    a. **사이클 주도 섹터 우선** (cycle_context.leader_sectors와 일치하는 섹터에 가중치 부여)
    b. 매크로 뉴스/체제에서 파악되는 구조적 순풍 (금리/정책/수급/기술 트렌드)
    c. 현재 포트폴리오의 섹터 편중 보완 (분산 효과)
    d. 체제별 PER/PBR 한도 충족 가능한 섹터
    e. 해당 체제의 방어적/공격적 성격에 부합하는 섹터 특성
31. 대표 종목 선정 기준:
    a. 해당 섹터 시가총액 상위 (유동성 확보)
    b. 체제별 밸류에이션 제한(PER {per_max}/PBR {pbr_max}) 충족 가능 종목
    c. 국내(KR)와 미국(US) 종목 혼합 가능
    d. 현재 보유 종목과 중복 금지
32. 포트폴리오 가중 평균 등급이 B 미만(weighted_avg score<16)이면 신규 섹터 추천은 **사이클 주도 섹터 1개로 제한** (전면 금지 아님). 기존 C/D 종목 정리 후 진입 권고.

{contrarian_section}매크로 체제: {regime} ({cash_guidance}){cash_warning}
{_format_cycle_context(cycle_ctx)}{user_comment_block}"""
...
def _build_prompt(
    ...
    user_comment(2026-05-07): 시스템 프롬프트 후미 가이드 블록 + user 메시지에 echo.
    """
    ...
    cycle_ctx = context.get("cycle_context") or {}
    system_prompt = _build_system_prompt(regime, cash_ratio, cycle_ctx, user_comment=user_comment)
    ...
    # 경기 사이클 국면 정보 추가
    if cycle_ctx and cycle_ctx.get("phase"):
        macro_summary["cycle_phase"] = cycle_ctx.get("phase")
        macro_summary["cycle_label"] = cycle_ctx.get("phase_label")
        macro_summary["leader_sectors"] = cycle_ctx.get("leader_sectors")
        macro_summary["cycle_confidence"] = cycle_ctx.get("confidence")

    # 뉴스 헤드라인 수집 → 섹터 추천 컨텍스트로 활용
    news_headlines = _get_macro_news_context()
    if news_headlines:
        macro_summary["news_headlines"] = news_headlines
...
def analyze_portfolio(
    ...
    1. OPENAI_API_KEY 확인
    2. 캐시 확인 (force_refresh=False일 때, 캐시 키는 잔고+코멘트 조합)
    3. 컨텍스트 구성 → 프롬프트 빌드 → OpenAI 호출
    4. 결과 캐싱 → 반환

    user_comment(2026-05-07): 사용자 가설을 시스템 프롬프트 + user 메시지에 전파.
    캐시 키에도 포함되어 동일 잔고+다른 코멘트 = 신규 보고서.
    """
    if not OPENAI_API_KEY:
        raise ConfigError("OPENAI_API_KEY가 설정되지 않았습니다.")

    cache_key = _compute_cache_key(balance_data, user_comment)
    ...
    context["cycle_context"] = _get_cycle_context()
    ...
    system_prompt, user_prompt = _build_prompt(
        context, macro_ctx, regime, regime_desc, user_comment=user_comment,
    )
