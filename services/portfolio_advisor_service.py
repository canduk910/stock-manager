...
def _format_cycle_context(cycle_ctx: dict | None) -> str:
    ...
    return f"경기 사이클 국면: {label} (신뢰도 {conf}%). {desc}. 주도 섹터: {sectors}. 섹터 추천 시 경기 사이클 주도 섹터를 우선 고려하세요."
...
29. 체제별 신규 섹터 추천 한도:
    ...
    - cautious: 최대 1개 섹터, 대표 종목 1개
    - defensive: 신규 섹터 진입 금지 (sector_recommendations = [])
30. 추천 섹터 선정 기준:
    a. 매크로 뉴스/체제에서 파악되는 구조적 순풍 (금리/정책/수급/기술 트렌드)
    b. 현재 포트폴리오의 섹터 편중 보완 (분산 효과)
    c. 체제별 PER/PBR 한도 충족 가능한 섹터
    d. 해당 체제의 방어적/공격적 성격에 부합하는 섹터 특성 (defensive→해당없음, cautious→방어 섹터 우선)
31. 대표 종목 선정 기준:
    a. 해당 섹터 시가총액 상위 (유동성 확보)
    b. 체제별 밸류에이션 제한(PER {per_max}/PBR {pbr_max}) 충족 가능 종목
    c. 국내(KR)와 미국(US) 종목 혼합 가능
    d. 현재 보유 종목과 중복 금지
32. 포트폴리오 가중 평균 등급이 B 미만(weighted_avg score<16)이면 신규 섹터 추천 금지 (규칙 G 동일 — 기존 C/D 종목 정리 우선).
