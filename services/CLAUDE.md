...
| `tax_service.py` | 해외주식 양도소득세: 3단계 동기화(CTOS4001R 환율+수수료 내장 → TTTS3035R 기간별 체결 → 로컬 DB) + **과거 5년 매수 자동 탐색**(FIFO 매수풀 부족 방지) + 환율 조회(yfinance+cache.db) + FIFO 전용 계산 + 연간 요약(기본공제250만/세율22%). `stock/tax_store.py` 래퍼 경유. |
