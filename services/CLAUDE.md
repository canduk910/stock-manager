...
| `order_us.py` | 해외주식 KIS API 주문 실행. **(2026-05-05 핫픽스)** `get_overseas_executions` + `get_overseas_open_orders` NASD 단일 → NASD/NYSE/AMEX 3개 거래소 순회. 체결조회는 KST 어제~오늘 명시(ET 경계 누락 방지). 중복 제거 키 `order_no::exchange`. |
