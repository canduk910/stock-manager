...
| `detail_service.py` | 재무 테이블 + PER/PBR 히스토리 + CAGR 종합 리포트. KR metrics에서 PBR/PER/ROE fallback 보충. **`get_bundle(symbol, market='auto')`** — DetailPage 마운트 시 필요한 모든 섹션(basic/financials/valuation/forward_estimates/summary)을 ThreadPoolExecutor로 병렬 수집. 부분 실패 시 `partial_failure: list[str]`로 보존. N+1 호출 패턴 제거. |
