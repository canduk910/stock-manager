...
| `guru_formulas.py` | **구루 투자 공식 모듈** (신규). Greenblatt Magic Formula(`calc_greenblatt`: ROIC+EY), Neff Total Return(`calc_neff`: EPS CAGR+배당/PER), 서준식 기대수익률(`calc_seo_expected_return`: ROE/PBR). 통합 패널(`calc_guru_panel`: 12점=3공식×4점). Value Trap 경고(`check_value_trap`: 5규칙). 체제별 파라미터(`REGIME_GURU_PARAMS`). routers/screener.py에서 사용. |
