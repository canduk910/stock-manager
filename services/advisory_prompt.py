"""AI자문 프롬프트 빌더 (순수 문자열 조립).

advisory_service.py에서 분리(2026-06-20 구조 리팩토링). GPT 시스템/유저 프롬프트 조립,
컨센서스/매크로/전략신호 섹션 생성, 보고서 JSON 파싱을 담당한다. 외부 I/O 없음(순수 함수).

도메인 주의 (margin-analyst 자문):
  본 모듈의 7점 등급 임계값·손절폭·grade_factor·Value Trap 규칙 문자열은 safety_grade.py(코드)
  + Pydantic 스키마(타입)와 '3중 일관성 필수'로 의도적으로 동일하게 유지된다. 이 문자열들은
  safety_grade.py와 중복처럼 보이지만 제거/상수화 금지 — 프롬프트 가독성 + 도메인 SSoT 정합성.
  체제×사이클 16셀 매트릭스(_CYCLE_REGIME_RULES)와 Graham 할인 임계값도 동일.

기존 호출 경로 호환: advisory_service 가 본 모듈의 함수를 re-export 하여
generate_ai_report() 및 모든 기존 import 경로가 그대로 동작한다.
"""
