...
- [주식현재가 일자별](#주식현재가-일자별) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-daily-price` (실전 TR_ID: `FHKST01010400`)
- [주식현재가 시세](#주식현재가-시세) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-price` (실전 TR_ID: `FHKST01010100`)
- [국내주식 시간외현재가](#국내주식-시간외현재가) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-overtime-price` (실전 TR_ID: `FHPST02300000`)
- [ETF 구성종목시세](#etf-구성종목시세) — `GET` `/uapi/etfetn/v1/quotations/inquire-component-stock-price` (실전 TR_ID: `FHKST121600C0`)
- [주식현재가 시간외시간별체결](#주식현재가-시간외시간별체결) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-time-overtimeconclusion` (실전 TR_ID: `FHPST02310000`)
- [NAV 비교추이(종목)](#nav-비교추이종목) — `GET` `/uapi/etfetn/v1/quotations/nav-comparison-trend` (실전 TR_ID: `FHPST02440000`)
- [주식현재가 시간외일자별주가](#주식현재가-시간외일자별주가) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-daily-overtimeprice` (실전 TR_ID: `FHPST02320000`)
- [국내주식 시간외호가](#국내주식-시간외호가) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-overtime-asking-price` (실전 TR_ID: `FHPST02300400`)
- [주식현재가 당일시간대별체결](#주식현재가-당일시간대별체결) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-time-itemconclusion` (실전 TR_ID: `FHPST01060000`)
- [주식현재가 시세2](#주식현재가-시세2) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-price-2` (실전 TR_ID: `FHPST01010000`)
- [주식일별분봉조회](#주식일별분봉조회) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-time-dailychartprice` (실전 TR_ID: `FHKST03010230`)
- [국내주식기간별시세(일/주/월/년)](#국내주식기간별시세일주월년) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice` (실전 TR_ID: `FHKST03010100`)
- [NAV 비교추이(일)](#nav-비교추이일) — `GET` `/uapi/etfetn/v1/quotations/nav-comparison-daily-trend` (실전 TR_ID: `FHPST02440200`)
- [주식현재가 호가/예상체결](#주식현재가-호가예상체결) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-asking-price-exp-ccn` (실전 TR_ID: `FHKST01010200`)
- [주식현재가 체결](#주식현재가-체결) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-ccnl` (실전 TR_ID: `FHKST01010300`)
- [주식현재가 회원사](#주식현재가-회원사) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-member` (실전 TR_ID: `FHKST01010600`)
- [NAV 비교추이(분)](#nav-비교추이분) — `GET` `/uapi/etfetn/v1/quotations/nav-comparison-time-trend` (실전 TR_ID: `FHPST02440100`)
- [주식현재가 투자자](#주식현재가-투자자) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-investor` (실전 TR_ID: `FHKST01010900`)
- [ETF/ETN 현재가](#etfetn-현재가) — `GET` `/uapi/etfetn/v1/quotations/inquire-price` (실전 TR_ID: `FHPST02400000`)
- [국내주식 장마감 예상체결가](#국내주식-장마감-예상체결가) — `GET` `/uapi/domestic-stock/v1/quotations/exp-closing-price` (실전 TR_ID: `FHKST117300C0`)
- [주식당일분봉조회](#주식당일분봉조회) — `GET` `/uapi/domestic-stock/v1/quotations/inquire-time-itemchartprice` (실전 TR_ID: `FHKST03010200`)
...
| FID_PERIOD_DIV_CODE | 기간 분류 코드 | string | Y | 32 | 'D : (일)최근 30거래일 <br>W : (주)최근 30주 <br>M : (월)최근 30개월' |
| FID_ORG_ADJ_PRC | 수정주가 원주가 가격 | string | Y | 10 | '0 : 수정주가미반영<br>1 : 수정주가반영<br>* 수정주가는 액면분할/액면병합 등 권리 발생 시 과거 시세를 현재 주가에 맞게 보정한 가격' |
...
| flng_cls_code | 락 구분 코드 | string | Y | 2 | '01 : 권리락 <br>02 : 배당락 <br>03 : 분배락 <br>04 : 권배락 <br>05 : 중간(분기)배당락 <br>06 : 권리중간배당락 <br>07 : 권리분기배당락' |
...
| iscd_stat_cls_code | 종목 상태 구분 코드 | string | Y | 3 | 51 : 관리종목<br>52 : 투자위험<br>53 : 투자경고<br>54 : 투자주의<br>55 : 신용가능<br>57 : 증거금 100%<br>58 : 거래정지<br>59 : 단기과열종목 |
...
| appsecret | 앱시크릿키 | string | Y | 180 | 한국투자증권 홈페이지에서 발급받은 appkey (절대 노출되지 않도록 주의해주세요.) |
| personalseckey | 고객식별키 | string | N | 180 | [법인 필수] 제휴사 회원 관리를 위한 고객식별키 |
| tr_id | 거래ID | string | Y | 13 | [실전투자/모의투자]<br>FHPST02310000 |
| tr_cont | 연속 거래 여부 | string | N | 1 | tr_cont를 이용한 다음조회 불가 API |
...
| FID_INPUT_ISCD | 입력 종목코드 | string | Y | 12 | 종목번호 (6자리)<br>ETN의 경우, Q로 시작 (EX. Q500001) |
...
| appsecret | 앱시크릿키 | string | Y | 180 | 한국투자증권 홈페이지에서 발급받은 appkey (절대 노출되지 않도록 주의해주세요.) |
| personalseckey | 고객식별키 | string | N | 180 | [법인 필수] 제휴사 회원 관리를 위한 고객식별키 |
| tr_id | 거래ID | string | Y | 13 | [실전투자/모의투자]<br>FHPST02320000 |
| tr_cont | 연속 거래 여부 | string | N | 1 | tr_cont를 이용한 다음조회 불가 API |
| custtype | 고객 타입 | string | N | 1 | B : 법인 <br>P : 개인 |
| seq_no | 일련번호 | string | N | 2 | [법인 필수] 001 |
| mac_address | 맥주소 | string | N | 12 | 법인고객 혹은 개인고객의 Mac address 값 |
...
| FID_INPUT_ISCD | FID 입력 종목코드 | string | Y | 12 | 종목번호 (6자리)<br>ETN의 경우, Q로 시작 (EX. Q500001) |
...
| mrkt_warn_cls_name | 시장 경고 구분 명 | string | Y | 40 | 특정 경우에만 데이터 출력<br>"투자환기" / "투자경고" |
| stck_sdpr | 주식 기준가 | string | Y | 10 |  |
...
| personalseckey | 고객식별키 | string | N | 180 | [법인 필수] 제휴사 회원 관리를 위한 고객식별키 |
| tr_id | 거래ID | string | Y | 13 | [실전투자/모의투자]<br>FHKST01010600 : 주식현재가 회원사 |
| tr_cont | 연속 거래 여부 | string | N | 1 | tr_cont를 이용한 다음조회 불가 API |
...
| Element | 한글명 | Type | Required | Length | Description |
| --- | --- | --- | --- | --- | --- |
| rt_cd | 성공 실패 여부 | string | Y | 1 | 성공 실패 여부 <br>성공 : 0   실패 : 0외 값 |
