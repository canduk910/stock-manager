...
| `financial_ratios.py` | **4축 재무비율 평가** (신규, 2026-06-19). `compute_ratio_analysis(income_stmt, balance_sheet, cashflow, metrics, sector_tier="general")` 순수 함수(외부 호출 0). 16비율 4축(활동성/성장성/수익성/안정성) → 비율당 0~4점 → 축 평균 ×25 = 0~100 → 5등급. 수익성·안정성=절대 임계값(한국 보정), 활동성·성장성=추세(정규화 회귀 기울기). 금융업 분기 + 결손 None skip + 진단 문구 자동 생성. 매매 액션 키 금지. advisory `ratio_analysis` 키로 노출 |
