...
| `kis_auth.py` | **KIS 인증 공통 모듈** (2026-07-04 F-1: `routers/_kis_auth.py`에서 이동 — services/stock의 라우터 역방향 import 17개소 교정). 토큰 관리(캐시 키 `(user_id, account_label)`)/hashkey/make_headers + 멀티 계좌 `get_kis_credentials(user_id, account_label)` + ContextVar(`_current_user_id`/`_current_account_label`). 구 경로 `routers/_kis_auth.py`는 sys.modules 별칭 shim — 동일 모듈 객체라 기존 import/monkeypatch 완전 호환. |
...
| `_dashboard_cache.py` | **워치리스트 dashboard 응답 캐시** (2026-05-03 Phase 2 QW-4). 사용자별 in-memory TTL **5s** (부분실패 3s) — **현재가 캐시 금지 도메인 원칙**, F5 dedup + t3.small swap thrashing 방지 한정 (2026-05-15 단축, 기존 60s/15s). 시세판 인메모리(장중 10s)와 일관된 정책. `(user_id, sorted_codes_hash)` 키. `add/remove_item / update_memo` 핸들러에서 invalidate. threading.Lock 보호. 멀티 인스턴스 확장 시 Redis 재설계 필요. |
| `watchlist_service.py` | `WatchlistService` — 관심종목 대시보드 + 상세 조회. ThreadPool max_workers=4 (t3.small OOM 방지, 2026-05-02 10→4). **(2026-05-03 Phase 2 QW-1/QW-3)**: `is_stale_from_dict()` dict 기반 fresh 판정으로 N+1 제거(SELECT 104→26). `partial_failure: list[str]` 메타필드 + `logger.debug→warning` 승격. **(2026-07-04 F-4)**: `_prefetch_kr_prices(kr_codes)` — KR 현재가를 `fetch_prices_batch` 1회 프리페치(외부 API N회→1회, 시세판과 경로 공유). `{close, change, change_pct}` 키 정규화, mktcap은 stock_info→metrics 소싱, 배치 누락 종목은 per-code `fetch_price` 폴백(partial_failure 시맨틱 보존). |
...
| `advisory_service.py` | 자문종목 데이터 수집 + OpenAI 리포트 생성. macro_cycle 통합 + cycle×regime 16셀 매트릭스 + 성장 보조등급 병기 (2026-05-02). **2026-06-20 분할**: 1861→800줄. 프롬프트 빌더→`advisory_prompt.py`, 챗봇→`advisory_chat.py` verbatim 이동 + re-export(공개 API 무변경) |
| `advisory_prompt.py` | **AI자문 프롬프트 빌더** (2026-06-20 advisory_service 분할). GPT 시스템/유저 프롬프트 구성 9함수. 7점 등급/손절/Value Trap 문자열은 safety_grade.py와 3중 일관성 필수. advisory_service가 re-export |
| `advisory_chat.py` | **AI자문 챗봇** (2026-06-20 advisory_service 분할). `chat_with_report()` stateless 챗봇(DB user_id 검증). advisory_service가 re-export |
...
| `macro_factor_model.py` | **거시 팩터 롤링 PCA** (신규, 2026-06-20). 순수 numpy(`np.linalg.svd`/`lstsq`, scikit-learn 미사용). 거시 10지표 7년 일별 시계열 → 504일 롤링 PCA(5 주성분, step=5) → 4종(5축 z-score 신호등·종목 거시베타+R²·설명력 추이·loadings). `build_and_cache_factor_model()` 일배치(KST 00:20, scheduler) → `macro_store.save_today("factor_model")`. 종목 베타 대상=watchlist∪advisory(0건이면 대표리스트 폴백). 무거운 계산은 배치 전용(요청 경로 금지, 캐시 miss 시 `status:"pending"`). |
| `safety_grade.py` | 7점 등급 (가치). `GRADE_FACTOR["C"]=0.25` C 부분진입 허용 (2026-05-02) |
| `financial_ratios.py` | **4축 재무비율 평가** (신규, 2026-06-19). `compute_ratio_analysis(income_stmt, balance_sheet, cashflow, metrics, sector_tier="general")` 순수 함수(외부 호출 0). 16비율 4축(활동성/성장성/수익성/안정성) → 비율당 0~4점 → 축 평균 ×25 = 0~100 → 5등급. 수익성·안정성=절대 임계값(한국 보정), 활동성·성장성=추세(정규화 회귀 기울기). 금융업 분기 + 결손 None skip + 진단 문구 자동 생성. 매매 액션 키 금지. advisory `ratio_analysis` 키로 노출. **2026-06-20 보정(ultrareview)**: 이자보상비율 `ie=None`(파싱 실패) None skip(무차입 0과 구분, 4점 false-positive 차단) / 자본잠식(`total_equity≤0`) 부채비율 1점 sentinel + `axes.stability.equity_deficit` 플래그 + "자본잠식" 진단 / ROE `metrics.roe` None 시 latest `net_income/total_equity×100` fallback |
...
`routers/advisory.py`에서 사용. `stock/advisory_fetcher.py`와 `stock/dart_fin.py`/`stock/yf_client.py`를 조합해 분석 데이터를 수집하고, OpenAI GPT-5.4로 종합 리포트를 생성한다.
...
#### `generate_ai_report(code, market, name, user_id=1, user_comment=None) → dict`
...
**사용자 코멘트 양면 평가** (2026-05-07): `user_comment` (Optional[str], 1000자 상한, 라우터에서 검증)을 전달하면 시스템 프롬프트 후미에 "동의/반박 양면 평가 + strength 1~10 + overall_stance 5단계" 가이드 블록이 append되고, GPT 응답 JSON에 `user_commentary_evaluation` 섹션이 포함된다. 등급/composite_score/action은 코멘트 무관 데이터 기반 유지(stance만 가설 평가용). 코멘트 None/공백 시 기존 동작과 동일.

**사용 모델**: `OPENAI_MODEL` 환경변수로 설정 (기본값: `gpt-5.4`). `max_completion_tokens=2500` 사용 (신규 모델 호환).

**프롬프트 구성**: 10년 손익/대차/현금흐름 + 계량지표(**배당수익률·주당배당금 포함**) + **사업 개요/사업부문 매출비중** + **52주 가격 위치** + **자본행위 분류**(CB/BW/유증/감자/자사주/M&A 자동 추출) + **장기(10년) 밸류에이션 사이클** + **경쟁사 비교(peers)** + 기술적 시그널 + 포워드 가이던스 + 분기 실적 + 7점 등급 사전 계산값 + 매크로 환경 + KIS 전략 신호 + 리서치 데이터(거시/실적일/경영진/공시/뉴스/섹터) + **12번 증권사 컨센서스 섹션** (`_build_consensus_section()`: defensive 체제 미표시 / cautious 50% 가중 감산 / KR 중앙 목표가·dispersion·momentum/US 등급 변경 이력). Value Trap 6번째 규칙(consensus_overheated). 사전 계산값(grade/score/진입가/손절가) 불변.

**시스템 프롬프트 — 미래지향/역발상 강화** (2026-05-01):
- "결정은 과거가 아닌 미래에 베팅" 원칙
- 역발상 4대 시그널: 과매도+펀더멘털 강건 / 사이클 턴어라운드 / 컨센서스 합의도 반박 / 자사주·내부자 매수
- catalyst 8종 식별 의무 (3개 이상 권장: 신규시장/제품/규제/M&A/자사주/배당정책/구조조정/사이클회복)
- peak-out 선행지표 검증 지침
- **8대 비판적 분석 항목** (기존 6대 + `미래성장동력` + `역발상관점`)
...
- 모두 `Optional` → 기존 응답 backward-compat

**3전략 프레임워크**: 시스템 프롬프트에 3가지 전략을 명시하여 GPT-5.4가 각 관점에서 분석.
