...
| `analyst_pdf.py` | 증권사 PDF 본문 추출+요약. `summarize_one(pdf_url)` — pdfplumber 첫 5페이지 → gpt-5.4 JSON 6항목(catalyst 2/risk 2/TP 근거/EPS 변경) → 300자. cache.db 영구. ai_gateway 시스템 호출(`user_id=None`, `service_name="analyst_summary"`). 실패 시 빈 문자열. |
...
| `semi_collectors/` | **반도체 사이클 선행지표 수집기 패키지** (2026-06-13 신규, Phase 1, 5종). `base.py` — `CollectorResult` dataclass + `apply_outlier_guard(curr, prev, pct=30.0)` 직전값 대비 ±30% 격리(지시서 2.1 sanity check) + `pct_change_or_none(curr, prev)`. `__init__.py` — `COLLECTOR_REGISTRY` dict + `INDICATOR_LABELS`(한글). 모든 수집기: `def collect() -> list[CollectorResult]` 단순 함수 패턴 (기존 `macro_fetcher`/`dart_fin` 컨벤션, `BaseCollector` 추상 클래스 미사용). 영속 시계열은 `db/models/semiconductor.py:IndicatorValue` 단일 출처, raw 응답만 `cache.py` TTL. |
| `semi_collectors/hyperscaler_capex.py` | 지표 2. SEC EDGAR companyfacts XBRL `PaymentsToAcquirePropertyPlantAndEquipment` × MSFT(0000789019)/GOOGL(0001652044)/AMZN(0001018724)/META(0001326801). FY 누계→Q4 derivation(직전 3분기 차감). 4사 분기 합산 USD + YoY. User-Agent `stock-manager/1.0 (contact=<config.SEC_EDGAR_USER_AGENT_CONTACT>)`. `value_meta = {unit:"USD", by_company:{...}, yoy_pct, quarter_fp}`. |
| `semi_collectors/memory_inventory.py` | 지표 4. `stock.dart_fin.fetch_quarterly_financials("005930"/"000660")` 재사용. BS `재고자산` + IS `매출원가` → `inventory_days = 재고/(매출원가/91)`. 매출원가 0/음수 시 None 반환 가드. `value = (samsung + hynix) / 2`, `value_meta = {unit:"days", samsung:{...}, hynix:{...}}`. |
| `semi_collectors/hbm_contracts.py` | 지표 5. `screener/dart.py` 주요사항보고 B 일일 스캔. 정규식 2단계 — 1차 `^단일판매.공급계약체결` + 2차 `keyword_regex`(임계값 테이블) + 발행회사 005930/000660 한정으로 false-positive 차감. `re.IGNORECASE`. `value_meta.rcept_no` 멱등성 가드. 매칭 시 INFO 즉시 (당일 빈 결과 캐시 금지 — 골프존 사례 컨벤션). |
| `semi_collectors/ai_ipo_tracker.py` | 지표 6. `config.AI_IPO_TICKERS` 화이트리스트(콤마 구분 예 `"RDDT,ARM"`) + `stock.yf_client.fetch_price_yf` 현재가. 공모가는 `semiconductor_thresholds.{ticker}/ipo_price`에 관리자 수동 시드(SEC S-1 자동 파싱 정확도 낮음). SEC efts.sec.gov full-text `"lock-up" forms=S-1,424B4` → "180-day lock-up" 정규식 해제일 추출(영구 캐시). -20% WARNING / D-7 INFO. 첫 5거래일은 outlier guard 우회. |
| `semi_collectors/market_breadth.py` | 지표 8. `screener.krx._find_latest_trading_day` + `pykrx.stock.get_market_ohlcv("KOSPI")` 전일/당일. ADR(20) = sum(20일 상승) / sum(20일 하락) — 일별 ORM에 누적 rolling. 삼성+SK 시총 / KOSPI 전체 = 집중도. 252거래일 신고치 비교. KOSPI 신고가 갱신일 AND ADR<0.8 → WARNING. KRX 로그인 자격증명(`KRX_ID`/`KRX_PASSWORD`) 필요. |
    ...
    "balance_sheet": [
        {
            ...
            "cash_and_equiv": ..., "receivables": ..., "payables": ...,  # payables=매입채무 (2026-06-19 신규, 매입채무회전율용)
            "inventories": ...,
            "ppe": ..., "intangibles": ...,
