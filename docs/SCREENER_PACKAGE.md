...
| 함수 | 설명 |
|------|------|
| `fetch_filings(start_date, end_date, *, corp_code=None)` | 기간별 정기보고서 제출 목록. `corp_code` 지정 시 해당 회사만 조회 + DART 정책상 검색기간 무제한. 미지정 시 전종목 조회(DART 정책상 3개월 초과 거부 — 호출자 책임). end_date < today인 경우만 캐시 사용(오늘 이상 날짜는 캐시 bypass — 당일 공시가 수시 추가되므로). 캐시 키는 corp_code별 분리. |

### `fetch_filings()` 동작

- `pblntf_ty=A` (정기공시) 단일 쿼리 — `pblntf_detail_ty` 미사용으로 중복 방지
- `corp_code` 파라미터: 있으면 DART API 호출 params에 포함되어 단일 회사 + 무제한 기간 조회. 없으면 DART가 3개월 초과 시 `status=100` "corp_code가 없는 경우 검색기간은 3개월만 가능합니다" 거부.
...
### 단일 종목 자문 데이터 수집 패턴 (research_collector)

`stock/research_collector.py:_collect_capital_actions`은 종목별 1년 공시를 수집한다. 호출 패턴:

```python
from stock.dart_fin import _fetch_corp_code
from screener.dart import fetch_filings

corp_code = _fetch_corp_code(stock_code)  # corpCode.xml 매핑(3,963개) 조회
if corp_code:
    filings = fetch_filings(start, end, corp_code=corp_code)  # 365일 OK
else:
    # 매핑 실패는 매우 예외적(corpCode.xml 다운로드 실패/신규 상장 직후 미반영 등)
    short_start = (date.today() - timedelta(days=90)).strftime("%Y%m%d")
    filings = fetch_filings(short_start, end)  # 90일로 단축 + 클라이언트 필터
```
...
| `dart_filings:{start}:{end}` | DART 정기보고서 목록 (end < today 조건 시에만 저장) |
