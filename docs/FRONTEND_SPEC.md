...
| `OrderForm` | 매매구분/가격/수량 입력 폼. `symbol`, `symbolName`, `market` props로 종목/시장을 외부에서 제어. 잔고 페이지 URL 파라미터 `defaultValues` prop으로 반영. 매수가능 조회 버튼. `externalPrice` prop → 지정가 자동 세팅(호가창 연동). `externalSide` prop → 매매방향 자동 세팅(호가 클릭 연동). **FNO**: `FNO_ORDER_TYPE_OPTIONS`(지정가/시장가/조건부지정가/최유리지정가) + `FNO_CONDITION_OPTIONS`(없음/IOC/FOK). `mapFnoOrderCodes(orderType, condition)` → `{nmpr_type_cd, krx_nmpr_cndt_cd, ord_dvsn_cd}` 자동 매핑. 가격 step=0.01. **(2026-05-08 KRX+NXT+SOR)** `KR_EXCHANGE_OPTIONS = [SOR(추천)/KRX/NXT]` 거래소 셀렉터(market==='KR'만 노출, SOR 기본). 통합(UN)은 시세 전용 코드라 셀렉터 미노출. `isSimulation` prop 시 SOR/NXT 비활성+안내. body.exchange 자동 포함. |
| `OrderbookPanel` | 실시간 호가창. `symbol`+`market` prop. **KR/FNO 모두 `useQuote(symbol, market, 'auto')` 훅 사용** (REST 폴링 제거). KR=KIS WS 10호가(실시간), FNO=KIS FNO WS 호가(실시간, 5 또는 10레벨), US=현재가만 표시(호가 미지원 안내). KR/FNO는 동일한 호가창 그리드 렌더링 공유(매도↑매수↓, 잔량 배경바). **매도호가(asks) 클릭 → `onPriceSelect(price, 'sell')`, 매수호가(bids) 클릭 → `onPriceSelect(price, 'buy')`**. 연결 상태 배지(녹색/회색). symbol 없으면 플레이스홀더 표시. `useMemo([asks, bids, market])`으로 `displayAsks` / `displayBids` / `maxVolume` 재계산 방지. `export default memo(OrderbookPanel)`으로 props 미변경 시 리렌더 차단. **(2026-05-08)** `useMarketClock()` 결합 — KR 종목 헤더에 거래소 라벨(🟦 통합 09:00~15:30 / 🟢 NXT 08~09 또는 15:40~20 / 🟧 KRX 15:30~15:40 / 휴장). `useQuote(symbol, market, 'auto')` 로 백엔드가 KST 4구간 자동 분기. NXT/통합 호가도 모두 10호가 표시. |
| `ExchangeBadge` | (신규 2026-05-08) 거래소 5종 배지 공용 컴포넌트. `exchange` prop(`KRX`/`NXT`/`SOR`/`SOR-KRX`/`SOR-NXT`) + `size`(`xs`/`sm`). KRX=■ 파란색, NXT=◆ 녹색, SOR=⚡ 보라색. NULL/legacy 값은 KRX 폴백. `OpenOrdersTable`/`ExecutionsTable` 거래소 컬럼 + 토스트에서 사용. |
...
| 컴포넌트 | 설명 |
|---------|------|
| `FundamentalPanel` | 기본적 분석 탭. **① ForwardEstimatesSection**: `forward_estimates` 있을 때만 표시 — EPS/매출/순이익 추정치 + 목표주가 + 투자의견 카드. ② 계량지표 카드 그리드 (PER/PBR/PSR/EV·EBITDA/ROE/ROA/부채비율/유동비율) ③ 손익계산서 테이블+BarChart (매출/영업이익/순이익 + `{fyYear}E`/`{fyYear+1}E` 추정 열, 인디고 스타일) ④ 대차대조표 테이블 ⑤ 현금흐름표 테이블+BarChart (영업CF vs FCF) ⑥ 사업별 매출비중 PieChart (KR: "AI추정" 배지). **⑦ RatioAnalysisSection(최하단, 2026-06-19)**: 4축 재무비율 평가 — RadarChart(활동성/성장성/수익성/안정성 0~100) + 축별 진단 카드 4개(점수 게이지 + 등급 색상 + 자동 진단 문구 + 클릭 시 소속 비율 펼침). `data.ratio_analysis` null/overall None → EmptyState. 금융업 활동성 축 N/A 회색. |
| `RatioAnalysisSection` | **4축 재무비율 평가** (신규, 2026-06-19). props `data.ratio_analysis`. `axes.{activity,growth,profitability,stability}.{score,grade,applicable,na_reason,diagnosis,ratios{<key>:{value,points,trend_pct}}}` + `overall_score/overall_grade`. Recharts `RadarChart`/`PolarGrid`/`PolarAngleAxis`/`Radar`. 등급 색상 5단계(우수 green/양호 teal/보통 yellow/주의 orange/위험 red). 매매 액션 문구 미표시. **2026-06-20 보정**: `PolarRadiusAxis domain={[25,100]}`(최저 점수 25=비율당 1점×25, 0~24 dead zone 제거) + "최저 25점" 캡션. 자본잠식 종목은 `axes.stability.equity_deficit` + "자본잠식" 진단 표기. EmptyState 문구 "재무비율 데이터 부족" SSoT. |
...
- 상위 탭 UI: 재무분석(FinancialTable) / 종합 리포트
- 종합 리포트 탭 내 서브탭 4개:
