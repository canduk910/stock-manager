...
| `RatioAnalysisSection` | **4축 재무비율 평가** (신규, 2026-06-19). props `data.ratio_analysis`. `axes.{activity,growth,profitability,stability}.{score,grade,applicable,na_reason,diagnosis,ratios{<key>:{value,points,trend_pct}}}` + `overall_score/overall_grade`. Recharts `RadarChart`/`PolarGrid`/`PolarAngleAxis`/`Radar`. 등급 색상 5단계(우수 green/양호 teal/보통 yellow/주의 orange/위험 red). 매매 액션 문구 미표시. |
