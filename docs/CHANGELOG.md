...
**배경**: 연세대 정보대학원 〈AI와 신용평가모형〉 강의의 "전통적 재무정보 4유형"(활동성/성장성/수익성/안정성 비율)을 신용평가(부실/위험 신호 탐지) 관점으로 차용하여, 관심종목 상세 → 종합리포트 → 기본적분석(FundamentalPanel) **최하단**에 16개 재무비율을 4축으로 평가하는 기능 추가. **추가 외부 API 호출 0건** — 기존 fundamental 응답(다년치 BS/IS/CF + metrics) 재가공.

**사용자 의사결정 (4건 확정)**:
1. 시각화: Recharts RadarChart(4축 0~100) 1장 + 축별 진단 카드 4개(점수 게이지 + 자동 진단 문구 + 기여 비율 펼침)
2. 채점 철학: 혼합 — 수익성·안정성=절대 임계값, 활동성·성장성=자기 시계열 추세
3. 매입채무회전율: DART 파서에 매입채무 계정 추가 → 활동성 4비율 완성
4. 수익성 4번째 비율 = ROA (도메인팀 권고로 부채상환계수 대체 — 영업CF/총부채는 향후 "현금흐름 건전성" 5번째 축으로 분리 권고)

#### 16비율 4축 구성

- **활동성**(추세 채점): 총자산회전율=매출/총자산, 매출채권회전율=매출/매출채권, 재고자산회전율=매출원가/재고자산, 매입채무회전율=매출원가/매입채무
- **성장성**(CAGR 채점): 매출액/영업이익/순이익/총자산 증가율
- **수익성**(절대 채점): 영업이익률, 이자보상비율=영업이익/이자비용, ROE, ROA
- **안정성**(절대 채점): 부채비율, 자기자본비율=자본/자산×100, 차입금의존도=(단기+장기차입금)/총자산×100, 유동비율

#### 채점 방법론 (혼합)

- 비율당 0~4점 → 축별 (유효 비율) 평균 ×25 = 0~100점 → 5등급(85+우수/70+양호/50+보통/30+주의/<30위험)
- **추세 판정**(활동성·성장성): 최근 3년 정규화 선형회귀 기울기 `slope/mean×100`. 2개년은 단순 변화율, <2점은 None. 회전율 분모=기말잔액
- **절대 임계값 한국 상장사 보정** (1차안 → 보정): 부채비율 `<50→<100`(자본집약 산업, 250%↑ 위험) / 자기자본비율 `>60→>50` / 유동비율 `>200→>150`(한국 중앙값 ~130%) / 이자보상비율 보합대 `2~5→3~5`(한계기업 3배) / 차입금의존도 신규 `<15/15~30/30~45/>45` / ROA 신규 `>7/3~7/0~3/<0`
- **진단 문구 자동 생성**: score<50 시 최저 점수 비율 지목 + 강의 라벨(활동성→"비효율적 경영 신호" / 성장성→"매출 부진 신호" / 수익성→"수익성 악화 신호" / 안정성→"자금조달 능력 저하")

#### 금융업 분기 (§D-5)

- `sector_tier ∈ {bank_holding, insurance, securities}`: 활동성 축 N/A(재고/COGS/매출채권 무의미), 수익성 ROE/ROA만, 안정성 자기자본비율만(insurance K-ICS >15/10~15/5~10/<5, bank_holding·securities BIS/NCR >12/8~12/4~8/<4)

#### 데이터 결손 정책 (§D-6)

- 비율 입력 None/분모≤0 → 해당 비율 None(skip, 0점 미부여). 축당 유효 비율 ≥1개면 산출(평균), 0개면 축 N/A. overall = 산출 축 단순 평균, 산출 축 0개면 "평가 불가"

#### 변경 파일

- **신규** `services/financial_ratios.py` — 순수 함수 `compute_ratio_analysis(...)` (외부 호출 0, safety_grade.py 패턴)
- **신규** `frontend/src/components/advisory/RatioAnalysisSection.jsx` — RadarChart + 진단 카드 4개 + EmptyState
- `stock/dart_fin.py` — `_BS_REGEX`에 `payables`(매입채무) 정규식 1줄 추가(receivables 대칭, CLAUDE.md 정규식 변형 흡수 컨벤션)
- `services/advisory_service.py` — `_attach_ratio_analysis()` 헬퍼 + `_collect_fundamental_kr/us` 응답에 `ratio_analysis` 키(부분 실패 격리)
- `frontend/src/components/advisory/FundamentalPanel.jsx` — 최하단 `<RatioAnalysisSection>` 통합(기존 섹션 순서/동작 불변)

#### 테스트

- 신규 `tests/unit/test_financial_ratios.py` / `test_dart_fin_payables_regex.py` / `test_advisory_ratio_analysis.py` — 정상기업/부실기업/금융업/데이터결손/진단 경계값 + 매매 액션 키 부재 회귀 가드. 신규 62 PASS, feature 전체 123 PASS, 프론트 빌드 OK

#### 안전 규칙 (§D-7, OrderAdvisor)

- 분석/진단 표시 전용 — 응답·진단·UI 전체에 매매 액션 키(buy/sell/order/recommendation/action) 부재(전용 회귀 테스트 2건 가드). 자동 주문 연동 없음. 체제 독립(재무비율 절대 임계값은 시장 체제와 직교)

#### 도메인 자문 결과 (요약)

- **MarginAnalyst**: 한국 상장사 절대 임계값 보정(부채비율/자기자본비율/유동비율/이자보상 한계기업 기준), ROA·차입금의존도 신규 수립
- **ValueScreener**: 회전율 분모=기말잔액(DART 결손 robust), 추세=정규화 회귀 기울기, 보합대 [-5,0] 비대칭(하락 민감), 매입채무 정규식 변형 흡수
- **MacroSentinel**: 4축 평가는 종목 내재 속성 → 체제 독립(혼합 금지)
- **OrderAdvisor**: 매매 트리거 절대 금지, 진단은 "신호·우려·관찰 권고" 서술만
