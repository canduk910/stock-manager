...
class TestWatchlistAdd:
    def test_add_and_get(self, db_session):
        ...
        ok = repo.add_item(1, "005930", "삼성전자", memo="반도체 대장", market="KR")
        db_session.commit()
        assert ok is True

        item = repo.get_item(1, "005930", "KR")
    ...
    def test_add_duplicate(self, db_session):
        repo = WatchlistRepository(db_session)
        repo.add_item(1, "005930", "삼성전자", market="KR")
        db_session.commit()
    ...
    def test_add_same_code_different_market(self, db_session):
        repo = WatchlistRepository(db_session)
        ok1 = repo.add_item(1, "005930", "삼성전자", market="KR")
        ok2 = repo.add_item(1, "005930", "Samsung", market="US")
        db_session.commit()
        assert ok1 is True
        assert ok2 is True
...
class TestWatchlistUpdateMemo:
    def test_update_memo(self, db_session):
        repo = WatchlistRepository(db_session)
        repo.add_item(1, "005930", "삼성전자", memo="old")
        db_session.commit()

        ok = repo.update_memo(1, "005930", "new memo")
        db_session.commit()
        assert ok is True
    ...
    def test_update_memo_nonexistent(self, db_session):
        repo = WatchlistRepository(db_session)
        ok = repo.update_memo(1, "999999", "test")
        assert ok is False
...
class TestWatchlistMarketFilter:
    def test_market_filter(self, db_session):
        repo = WatchlistRepository(db_session)
        repo.add_item(1, "005930", "삼성전자", market="KR")
        repo.add_item(1, "AAPL", "Apple", market="US")
        db_session.commit()

        kr = repo.get_item(1, "005930", "KR")
        assert kr is not None
        assert kr["market"] == "KR"

        us = repo.get_item(1, "AAPL", "US")
        assert us is not None
        assert us["market"] == "US"

        # 다른 마켓으로 조회하면 None
        assert repo.get_item(1, "005930", "US") is None
