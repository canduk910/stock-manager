...
class TestAdvisoryStock:
    def test_add_stock(self, db_session):
        ...
        ok = repo.add_stock("005930", "KR", "삼성전자")
        db_session.commit()
        assert ok is True

        stock = repo.get_stock("005930", "KR")
    ...
    def test_add_stock_case_insensitive(self, db_session):
        ...
        repo = AdvisoryRepository(db_session)
        repo.add_stock("aapl", "us", "Apple")
        db_session.commit()

        stock = repo.get_stock("AAPL", "US")
        assert stock is not None
    ...
    def test_remove_stock(self, db_session):
        ...
        ok = repo.remove_stock("005930", "KR")
        db_session.commit()
        assert ok is True
    ...
    def test_remove_stock_nonexistent(self, db_session):
        repo = AdvisoryRepository(db_session)
        ok = repo.remove_stock("999999", "KR")
        assert ok is False
...
class TestAdvisoryCache:
    def test_save_cache(self, db_session):
        ...
        repo.save_cache("005930", "KR", fundamental, technical)
        db_session.commit()

        cache = repo.get_cache("005930", "KR")
    ...
    def test_save_cache_update(self, db_session):
        repo = AdvisoryRepository(db_session)
        repo.save_cache("005930", "KR", {"rev": 100}, {"rsi": 50})
        db_session.commit()
        first_cache = repo.get_cache("005930", "KR")
        first_updated = first_cache["updated_at"]

        # 업데이트
        repo.save_cache("005930", "KR", {"rev": 200}, {"rsi": 60})
        db_session.commit()
...
class TestAdvisoryReport:
    ...
    def test_get_latest_report(self, db_session):
        repo = AdvisoryRepository(db_session)
        repo.save_report("005930", "KR", "gpt-4o", {"v": 1})
        repo.save_report("005930", "KR", "gpt-4o", {"v": 2})
        db_session.commit()

        latest = repo.get_latest_report("005930", "KR")
        assert latest["report"]["v"] == 2

    def test_get_latest_report_nonexistent(self, db_session):
        repo = AdvisoryRepository(db_session)
        assert repo.get_latest_report("NOEXIST", "KR") is None

    def test_list_reports(self, db_session):
        repo = AdvisoryRepository(db_session)
        for i in range(5):
            repo.save_report("005930", "KR", "gpt-4o", {"v": i})
        db_session.commit()

        reports = repo.get_report_history("005930", "KR", limit=3)
    ...
    def test_report_summary_dict(self, db_session):
        repo = AdvisoryRepository(db_session)
        repo.save_report("005930", "KR", "gpt-4o", {"big": "data"},
                         grade="B+", grade_score=20, schema_version="v2")
        db_session.commit()

        history = repo.get_report_history("005930", "KR")
