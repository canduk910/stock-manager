...
| **관심종목** | 종목 자동완성 검색 추가. 시세·재무·섹터 대시보드. 종목 상세(재무 10년/4년·밸류에이션·종합 리포트). **2026-05-12 N+1 해소** — `/api/watchlist/batch-details` + `/api/detail/{code}/bundle` ThreadPool 병렬, 대시보드 10~20s → 2~5s. **2026-06-01 현재가 캐시 금지 강화** — 관심종목 목록·상세의 현재가는 6개 캐시 레이어(market:price/yf:price/market:detail/dashboard_cache/prices_batch in-memory/stock_info 영속) 모두 우회, 매 새로고침마다 외부 API(yfinance/KIS) 직접 호출. 메타(섹터·52주·PER·PBR)만 6h 캐시 유지. |
| **분석** ▼ | 매크로 분석(12섹션: 경기국면·지수·**투자자별 수급(코스피/코스닥, 개인/외국인/기관 일별+누적)**·심리·금리차·신용스프레드(HY OAS 백분위 5단계 하워드 막스 시계추 + IG OAS·HY-IG 보조)·환율·원자재·섹터히트맵·**뉴스(4h 공유 캐시)**·**투자자(KST 일별 공유 캐시)**·**반도체 사이클 모니터링(2026-06-13 신규, Phase 1, 5종 선행지표: 하이퍼스케일러 캐펙스/메모리 재고일수/HBM 장기공급 공시/AI IPO 추적/시장폭, 종합 GREEN/YELLOW/RED 신호 + 상세 모달 + 상태 변경 시 토스트 알림)**) / 종목 스크리너(섹터·52주 하락률 필터 포함) / 공시 조회. **2026-05-12 pre-warm** — KST 00:05 cron으로 매크로 GPT 자동 적재 (자정 후 첫 사용자 즉시 응답) |
| **포트폴리오** | 잔고 요약 + AI 자문(역발상 매수 전략 포함) + 자산배분 차트. **2026-05-17 UI 개선** — 종목별 수익률 차트 전체 종목 표시(이전 상위 15개만) + 동적 height·Y축 너비 자동(종목명 잘림 차단). 섹터 진단 도넛 파이차트 + 우측 범례 두 그룹(① 보유 섹터 — 편중⚠/적정✓/부족↓ 색상 배지 / ② ⭐ 신규 편입 추천 — 포트폴리오 미보유 섹터만 타이밍·목표비중 표시) |
| **매매** ▼ | 주문(국내/해외/FNO, 실시간 호가창, **2026-05-08 KRX+NXT+SOR 통합** — 거래소 셀렉터 + 시간대 자동 분기, **2026-05-08~09 해외매매화면 풀패키지 개선** — 미국 10단계 호가창(KIS WS 우선 + REST 폴백) + 현재가 상세(시/고/저/거래량/52주) + USD/KRW 환율 동시 표시 + ET 기반 미국 거래시간 라벨(프리/정규/애프터/휴장+공휴일 30일)) / 잔고 조회 / 양도세(FIFO 자동 계산+시뮬레이션). **2026-05-15 KIS 멀티 계좌** — 사용자 1명이 여러 KIS 계좌(주식/연금/IRP/ISA 등) 등록 + 전체 자산 합산 평가 + 잔고 탭(전체/계좌별) + 주문 폼 계좌 드롭다운 + 체결통보 라벨링 |
| **백테스트** | KIS AI Extensions MCP(Lean) 연동(10 프리셋+커스텀 YAML) + **자체 Python 일봉 엔진** — 4개 KR 단기/추세 전략(상한가 모멘텀/변동성 돌파/20일 신고가 스윙/롱테일 변동성) 1~10종목 포트폴리오 균등 배분. **2026-05-12 UX 개선** — 상단 "단위: 종목 / 포트폴리오" segment 토글이 전략 탭 자동 제한(종목→빌더·MCP·커스텀 / 포트폴리오→로컬프리셋 단독) + 이력 테이블에 "포트폴리오 (N종목)" 표시 + 상세 모달(종목 칩 리스트 + 종목별 기여도 테이블 + 파라미터 JSON) |
| **보고서** | 일일 투자 보고서 / 추천 이력 / 성과 통계 |
| **관리자** | 사용자 CRUD(역할/비밀번호/방문수) / AI 사용량·한도·감사 로그 / 페이지별 이용 통계(호출수·평균/p95 latency·일별 시계열). **2026-05-17 신규** — 사용자별 일별 접속현황 모달(`/admin/users` 방문수 셀 또는 "이력" 버튼 → 7/30/90/180일 PV 막대 + 고유 path 라인 + 상위 path 리스트) |

### AI자문 (DetailPage 종합리포트 탭, 서브탭 5개)
- **기본적 분석**: 재무제표 3종 (손익·대차·현금흐름) + 계량지표 (PER·PBR·ROE·ROA·PSR·EV/EBITDA)
- **기술적 분석**: 타임프레임 선택 (15분/60분/1일/1주) + 캔들차트·볼린저밴드·MACD·RSI·Stochastic
- **AI 리포트**: OpenAI GPT-5.4 종합투자의견 (매수/중립/매도) + 기술적 시그널 + 리스크·투자포인트
- **수급/투자자** (2026-05-12 신규, 국내 종목 전용): KIS OpenAPI `FHPTJ04160001` — 일별 개인/외국인/기관(11종 분해) 순매수 + 누적 라인 + 매수/매도 분리 토글. 상단 안전 고지(매매 신호 단독 사용 금지, Graham 원칙). 장중 호출 시 직전 영업일까지 자동 후퇴(KST 15:40 cutoff)
- **증권사 컨센서스 통합**: 네이버 증권 리서치 PDF 본문을 GPT-5.4로 6항목(catalyst/risk/TP 근거/EPS 변경) 요약 → AI 프롬프트에 합류. 중앙 목표가·dispersion·momentum 5단계·과열 경고·6개월 추이. 체제별 차등(defensive 미표시 / cautious 50% 감산). US는 yfinance 등급 변경 이력. 시스템 호출 모드(유저 일일 쿼터 미차감)
...
- **주문발송**: 상단 시장+종목 검색바(공유) + [신규주문 | 정정/취소] 서브탭. xl 이상에서 2컬럼(왼쪽=호가창, 오른쪽=주문폼). 호가 클릭 시 매매방향+가격 자동 입력. 하단에 가격 차트 패널(캔들+MA5/20+볼린저밴드+거래량).
  - **(2026-05-08 KRX+NXT+SOR)** KR 종목 주문폼에 **거래소 셀렉터**(SOR/KRX/NXT) 추가. 기본값 SOR(KRX/NXT 중 가격 유리한 곳 자동 라우팅). 호가창 헤더에 현재 시간대 거래소 라벨 자동 표시:
    - 🟦 통합 09:00~15:30 (KRX+NXT 통합시세, `H0UNCNT0`/`H0UNASP0`)
    - 🟧 KRX 15:30~15:40 (KRX 동시호가/마감)
    - 🟢 NXT 08:00~09:00 (프리오픈) / 15:40~20:00 (시간외)
    - 휴장 (그 외)
  - 모의투자(`openapivts`) 환경에서는 SOR/NXT 비활성화(KRX 강제). 통합(UN)은 시세 전용 코드라 주문값으로는 노출 안 됨.
- **미체결**: 미체결 주문 조회. API 발주분만 정정/취소 가능. 거래소 컬럼(■KRX/◆NXT/⚡SOR/⚡SOR→KRX/⚡SOR→NXT) 표시.
- **체결내역**: 당일 체결 내역. 거래소 컬럼 표시. 체결 토스트에도 거래소 prefix(`[⚡SOR-KRX] 체결: 삼성전자 매수 10주 @ 71,500원`).
...
1. **1~10개 종목** 칩(chip) 입력 → 균등 배분 포트폴리오 시뮬레이션 (슬롯 가득 차면 신규 신호 스킵)
2. **4개 KR 단기/추세 전략** 중 선택:
   - **상한가 모멘텀**(`momentum`): 전일종가 +29%↑ AND 종가<전일×1.30 → 당일 종가 매수 → 익일 시가 매도 → 손절 -7.5%
   - **변동성 돌파**(`volatility_breakout`): 시가+전일Range×K20 돌파 → 타겟가 매수 → 당일 종가 청산 → 손절 -3%. **K20 = 직전 20일 평균 노이즈 비율(`1 - |Close-Open|/(High-Low)`)** — 횡보장 진입 장벽↑·추세장 빠른 추격
   - **20일 신고가 스윙**(`donchian_swing`): 20일 신고가 + 60EMA 우상향 + 거래대금 1.5배 + 갭<+3% → 당일 종가 매수 → 20일 저가 이탈 매도 → 손절 -7%
   - **롱테일 변동성**(`long_tail_volatility`): VB 매수 + 전일대비 +3% 추가필터 → 당일 종가 매도, 단 +29% spike 시 익일 시가 매도 → 손절 당일 -3%/익일 -5%
3. 동기 응답으로 즉시 결과 표시: 통합 자본곡선 + **종목별 기여도 카드** + 거래내역(symbol 컬럼 포함) + 8개 메트릭

> **일봉 단순화 한계**: intraday 트레일링·15:20 강제청산은 생략. 손절은 일봉 저가 ≤ 손절가일 때 손절가 체결로 가정. ±수% 정밀도 오차 수용.
...
4. **AI자문** 서브탭 → (선택) **사용자 의견 입력박스**에 가설 작성(1000자) → **[AI분석 생성]** 버튼 → GPT-5.4 종합투자의견 생성 (10~30초)
   - 코멘트를 입력하면 보고서 상단에 **동의/반박 양면 평가 카드**(stance 5단계 배지 + 좌(👍 녹색)/우(👎 빨강) 2컬럼 + 항목별 strength 1~10 게이지) 추가
   - 코멘트 미입력 시 기존 분석 흐름과 동일
...
| 계좌 잔고·주문·국내 실시간 호가 | 한국투자증권 OpenAPI | KIS API 키 필요. WS 끊김 시 REST 자동 fallback. **(2026-05-08)** KRX+NXT 통합시세 + SOR 활용. 시간대 4구간 자동 분기. 신 TR_ID(`TTTC0012U/0011U/0013U/0084R/0081R`) + EXCG_ID_DVSN_CD. 통합 명세는 `docs/kis/`(337개 API 마크다운) 참조. |
| 해외주식 실시간 시세 | [Finnhub](https://finnhub.io) WS | `FINNHUB_API_KEY` 설정 시 활성화. 무료 30 심볼. 미설정 시 yfinance 폴링 |
| AI 투자의견 | [OpenAI API](https://platform.openai.com) (GPT-5.4) | `OPENAI_API_KEY` 필요. 모델 변경 가능 |
...
│   ├── advisory_service.py#   AI분석 + GPT-5.4 리포트
│   ├── watchlist_service.py#  관심종목 대시보드
│   ├── detail_service.py  #   재무/밸류에이션/종합리포트
│   ├── reservation_service.py # 예약주문 스케줄러 (20초 폴링)
│   ├── macro_service.py   #   매크로 분석 (10섹션: 지수/심리/금리/신용/환율/원자재/섹터/국면/뉴스/투자자)
│   ├── macro_cycle.py     #   경기 사이클 국면 판단 (4단계, 5지표 가중합산)
...
| **현재가 / 일봉 / 15분봉** | ✅ | **(2026-05-08)** KIS OpenAPI 우선(`HHDFS00000300`/`HHDFS76240000`/`HHDFS76950200`) + yfinance 자동 fallback. 거래소 자동 매핑(NAS→NYS→AMS, `stock_info.exchange` 영속) |
| **호가창 (10단계)** | ✅ | **(2026-05-08~09)** 매매화면 호가창 — KIS WS HDFSASP0 우선 + REST `HHDFS76200100` 폴백 자동 전환. 호가 클릭→가격 자동 입력. 응답 단계 부족 시 동적 표시 |
| **현재가 상세 (시/고/저/거래량/52주)** | ✅ | **(2026-05-08)** KIS `HHDFS76200200`. OrderbookPanel 헤더에 표시 |
| **미국 거래시간 라벨** | ✅ | **(2026-05-08~09)** ET 기반 4구간(프리/정규/애프터/휴장) + DST 자동(`Intl.DateTimeFormat`) + NYSE/NASDAQ 정규 공휴일 30일(2026~2028) |
| **매수가능 USD/KRW 환율 동시 표시** | ✅ | **(2026-05-08)** OrderForm 매수가능 필드에 `$X (₩X) · 환율 ₩1,XXX/USD` |
| 대시보드 시세·재무·배당수익률 | ✅ | KIS 시세 + yfinance 재무 (15분 지연) |
| 종목 상세 재무 (최대 4년) | ✅ | yfinance 기준 (KIS 미제공) |
| 종목 상세: PER/PBR/ROE/배당수익률/주당배당금/52주 | ✅ | yfinance (KIS 미제공) |
| 공시 조회 (SEC 10-K/10-Q) | ✅ | 키 불필요 |
| AI자문 | ✅ | yfinance 재무 3종 + GPT-5.4 |
| PER/PBR 히스토리 차트 | ✅ | 분기 EPS/BPS + 일별 주가 기반 추정 (yfinance) |
| 스크리너 | ❌ | 국내 전용 |
| 지원 시장 | US | NASDAQ/NYSE/AMEX |
| 모의투자 해외거래 | ❌ | KIS 정책상 실전 한정 |
...
| 명령어 작성 | **Haiku** | (해당 사용자 정의 에이전트 없음 — `/model haiku` 수동 전환) |

각 에이전트 정의 파일 frontmatter `model:` 필드로 라우팅. **모든 비-사소 요청은 부서장**(`asset-dev` 스킬 또는 `Agent(subagent_type="department-head")`)을 단일 진입점으로 사용합니다. 메인 에이전트가 12명 하위 에이전트를 부서장 우회로 직접 호출하는 것은 금지(`CLAUDE.md` 하네스 섹션 「기본 라우팅(필수)」 참조).
...
- **모든 변경 이력은 [`docs/CHANGELOG.md`](docs/CHANGELOG.md) 단일 출처**. 최근 결함 수정(시계/WS URL 등)도 동 문서에 phase 단위 기록됩니다
