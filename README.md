...
| **분석** ▼ | 매크로 분석(10섹션: 경기국면·지수·심리·금리차·신용스프레드(HY OAS 백분위 5단계 하워드 막스 시계추 + IG OAS·HY-IG 보조)·환율·원자재·섹터히트맵·뉴스·투자자) / 종목 스크리너(섹터·52주 하락률 필터 포함) / 공시 조회 |
...
| **백테스트** | KIS AI Extensions MCP(Lean) 연동(10 프리셋+커스텀 YAML) + **자체 Python 일봉 엔진(2026-05-07 신규)** — 4개 KR 단기/추세 전략(상한가 모멘텀/변동성 돌파/20일 신고가 스윙/롱테일 변동성) 1~10종목 포트폴리오 균등 배분 백테스트 |
