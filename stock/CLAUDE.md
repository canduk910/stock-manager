...
| `dart_client.py` | **OpenDART HTTP 공용 클라이언트 (2026-07-04 F-5 신설).** `dart_get(url, params, timeout=30, retries=3)` — Connection: close 헤더 + ConnectionError 지수 재시도(3회 시도, 대기 1s/2s — screener `_dart_get` 정책 승격). 전송 레이어만 공용화 — 파라미터 구성/응답 파싱/status 해석은 호출 모듈(screener/dart.py, dart_segments.py, semi_collectors/hbm_contracts.py) 유지. |
...
| `sec_filings.py` | SEC EDGAR 미국 10-K/10-Q. User-Agent 연락처는 `config.SEC_EDGAR_USER_AGENT_CONTACT`(2026-06-13: 하드코딩 `contact@example.com` 제거, SEC 정책 정상 준수). 미설정 시 `ConfigError(503)`. |
| `semi_collectors/` | 반도체 사이클 선행지표 수집기 패키지(Phase 1, 5종). 단순 함수 패턴(`def collect() -> list[CollectorResult]`) — `BaseCollector` 추상 클래스 미사용. `base.py` (`CollectorResult` dataclass + `apply_outlier_guard(curr, prev, pct=30.0)` 직전값 대비 ±30% 격리 + `pct_change_or_none`) / `__init__.py` `COLLECTOR_REGISTRY` dict + `INDICATOR_LABELS` 한글 라벨. 수집기: `hyperscaler_capex.py`(지표 2, SEC EDGAR companyfacts XBRL 4사 분기 합산, FY→Q4 derivation) / `memory_inventory.py`(지표 4, `dart_fin.fetch_quarterly_financials` 재사용, 재고일수=재고/(매출원가/91)) / `hbm_contracts.py`(지표 5, `screener/dart.py` 주요사항보고 B 일일 스캔 + 정규식 2단계(`^단일판매.공급계약체결` AND `keyword_regex`) + 발행회사 005930/000660 한정 + `value_meta.rcept_no` 멱등성) / `ai_ipo_tracker.py`(지표 6, `config.AI_IPO_TICKERS` 화이트리스트 + yfinance 현재가 + 공모가 수동 시드 + SEC efts.sec.gov 락업 정규식 D-7 추출) / `market_breadth.py`(지표 8, pykrx ADR(20) + 삼성·SK 시총 집중도 + 252거래일 신고치 비교). 영속 시계열은 신규 ORM `indicator_values` 단일 출처, raw 응답만 `cache.py` TTL(SEC 24h/pykrx 6h/yf 30m). |
| `macro_fetcher.py` | 매크로 데이터: yfinance 지수/VIX/버핏/F&G. **`get_vix_spot() -> float\|None` VIX 스팟 단일 진입점 (2026-07-04 F-6, macro-sentinel CAUTION 승인)** — 공유 캐시 `macro:vix_spot` TTL 10분(0.17h) 고정(VIX>35 체제 오버라이드 즉각 반응 보존, 상향 금지), 실패 시 None+캐시 미저장(폴백값 오염 방지). 소비처: fetch_vix(표시 dict)/calc_fear_greed(폴백 20은 로컬만)/yf_client.fetch_macro_indicators. `services/macro_factor_model`의 ^VIX 7년 시계열은 용도가 달라 통합 대상 아님 + 금리곡선(^IRX/^FVX/^TNX/^TYX) + FRED HY OAS(BAMLH0A0HYM2) + IG OAS(BAMLC0A0CM) + 환율 4쌍 + 원자재 5종 + 섹터 ETF(US 11 + KR 14 — KODEX IT(157490) 추가, 네이버/카카오/삼성SDS/엔씨/넷마블 추적). FRED CSV 실패 → JSON API(`FRED_API_KEY`) → 7일 stale 순. **HY OAS 백분위 5단계** + OAS>10% 절대 안전장치. 2026-04~ ICE BofA 라이센스 변경으로 FRED ~3년치만 반환 → `oas_history_store` FIFO 누적 store에 매일 +1일 머지(7년 후 자연 10년 시계열 도달). 응답 `oas_stats{p10..p95}` / `oas_percentile` / `oas_zscore` / `oas_history_10y` / `oas_history_5y` / `ig_current` / `ig_history_10y` / `hy_ig_spread{,_history_10y}` / `partial_failure`. 캐시 24h(키 v8). `_compute_sma20_trend_days()` / `_compute_intensity_zscore(±3 cap)` 산점도 헬퍼. credit `direction`은 `oas_momentum_6m` 부호 기반 (HYG/LQD ETF 폐기, HYG 프록시 신뢰도 부족으로 미채택). |
