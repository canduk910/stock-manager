...
| `strategy_store.py` | 백테스트/전략 CRUD. `db/repositories/backtest_repo.py` 위임 래퍼. `save_backtest_job()`/`save_backtest_result()`/`update_job_status()`/`delete_job()`/`get_latest_backtest_metrics()`/`get_job_history()`. |
