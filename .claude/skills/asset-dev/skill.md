---
name: asset-dev
description: "투자 자동화 시스템 개발 오케스트레이터. 투자 파이프라인 서비스, APScheduler, Telegram 봇, 포트폴리오 대시보드, 리밸런싱, 리스크 모니터링, 성과 추적 등을 개발한다. 도메인 전문가 에이전트(MacroSentinel/MarginAnalyst/OrderAdvisor/ValueScreener)에게 투자 로직을 자문받으며 FastAPI+React 코드를 작성한다. '파이프라인 만들어줘', '시스템 개발', '자동화 구현', '대시보드 만들어줘', 'Telegram 연동', '스케줄러 설정' 등 시스템 개발 요청 시 반드시 이 스킬을 사용할 것."
---
