---
name: refactor-audit
description: "코드 구조 감사 + 도메인 인지 리팩토링 오케스트레이터. 코드 비대/중복/레이어 위반/일관성 결여를 감사하고, 도메인 전문가에게 '왜 이렇게 되어있는가'를 확인한 후, 도메인 정합성을 보존하면서 리팩토링을 실행한다. '리팩토링 해줘', '코드 정리', '구조 개선', '중복 제거', '코드 감사', '아키텍처 리뷰' 등의 요청 시 반드시 이 스킬을 사용할 것. 새 기능 개발(→ asset-dev)과는 구분된다."
---
